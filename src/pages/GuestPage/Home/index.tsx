        ...
        <div className="xl:col-span-8 flex flex-col gap-[32px] ">
          ...
          >
            ...
            {contributions.length === 0 ? (
              ...
              contributions.map((contribution: any, contributionIndex: number) => (
                  ...
                  <div className="md:col-span-10 flex flex-col gap-[7px]">
                    <div className="flex gap-2 items-center">
                      ...
                      <div className="relative image-stack bottom-3">
                        {contribution.images.slice(0, 4).map((image: any, index: number) => (
                          <div 
                          key={`${image}-${index}`}
                            style={{
          ...
          >
            ...
            <Formik
              ...
              onSubmit={(values: any, { resetForm }: any) => handleFormSubmit(values, resetForm)}
            >
              {({ values, handleSubmit, handleChange, setFieldValue }: any) => (
                    ...
                    <div>
                      {isFuneral && (
                        <div className="flex gap-[15px] md:gap-[20px]">
                          {funeral.map((funeralItem: any, index: number) => (
                            <div
                              key={`${funeralItem.type}-${index}`}
                  ...
                  <div className="flex flex-col border border-solid border-primary rounded-md px-[26px] py-[18px] bg-white leading-5 text-[14px] gap-[15px]">
                    ...
                    {values.TributeImages.length > 0 ? (
                      <div className="grid grid-cols-3 lg:grid-cols-6 flex-wrap gap-[10px]">
                        {values.TributeImages.map((tributeimage: any, index: number) => (
                          <div key={`${tributeimage}-${index}`} className="col-span-1 p-2 rounded-lg overflow-hidden bg-indigo-100">
                            <img
                              key={`${tributeimage}-${index}`}
        ...
        <div className="lg:col-span-4 flex flex-col gap-[32px]">
          ...
          <div className="bg-white p-[30px] rounded-md flex flex-col items-center gap-4">
            ...
            <div className="grid grid-cols-2 gap-[28px]">
              {WishItems.map((items: any, index: number) => (
                <div
                  key={`${items.image}-${index}`}
            ...
            <div className="mx-auto flex flex-col justify-center gap-[10px] w-[100%]">
              ...
              <div className="grid gap-[24px] gap-x-[24px] grid-cols-2 md:grid-cols-4 xl:grid-cols-4 w-full">
                {images.map((image: any, index: number) => (
                  <div
                    key={`${image}-${index}`}
                    className="w-[100px] h-[100px] lg:w-[60px] lg:h-[60px] 2xl:w-[100px] 2xl:h-[100px] rounded-lg bg-[#D9D9D9]">
                    {image}
                  </div>
                ))}
              </div>
