...
function TributeContributions() {
    ...
    return ( 
        <div className="-mt-6 mb-6 h-[30rem] overflow-y-sroll">
            ...
            { contributions.map(contribution => (
                ...
                onMouseEnter={() => showDeleteIcon(contribution.id)} onMouseLeave={() => hideDeleteIcon(contribution.id)} key={contribution.id}>
                    <div className="flex">
                        <p className="bg-gray-200 w-8 h-8 -mt-1 rounded-full flex items-center justify-center font-semibold mr-3 tracking-tighter text-[13px]">{contribution.initial}</p>
                        <div className={contribution.read ? "text-sky-700" : "text-black"}>
                            <h4 className="text-[12px] mb-1 font-semibold">{contribution.fullName}</h4>
                            <p className="text-[10px] max-w-[315px] leading-snug">{contribution.description}</p>
                        </div>
                    </div>
