...
const TableComponent: React.FC<TableComponentProps> = ({
  ...
  const handleActionClick = (index: number, action: any) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return tableData.filter((row: any) => row.status.toLowerCase().includes(filterStatus.toLowerCase()));
    ...
    <>
      <div className="bg-white rounded-2xl border pb-5">
        ...
        <table {...getTableProps()} className="w-full ">
          ...
          <tbody {...getTableBodyProps()}>
            {tablePage.map((row: any, i: number) => {
              prepareRow(row);
              return (
                <tr
                  {...row.getRowProps()}
