                ...
                <div key={index} className="w-full py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className=" flex flex-col gap-2 ">
                    <CustomInput
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFieldValue(`items.${index}.itemName`, e.target.value)}
                      type="text"
                        ...
                        <button
                          onClick={() => {
                            if (values.items.length === 1) {
                              ...
                              setFieldValue(
                                "items",
                                values.items.filter((_: any, i: number) => i !== index),
                              );
                            }
                          }}
                          type="button"
              ...
              <div className="mb-4">
                {values.items.length >= 10 ? (
                  ...
                  <motion.div whileHover={{ scale: 0.95 }}>
                    <div
                      className="bg-white text-primary flex items-center justify-center font-bold text-xl border border-gray-700 hover:cursor-pointer"
                      onClick={() => {
                        const newItem = { img: "", itemName: "", price: "" };
              ...
              <motion.div whileHover={{ scale: 0.98 }}>
                <button
                  className={` py-3 text-primary border border-primary rounded-md md:w-[167px] hover:bg-primary hover:text-white
                    ${values.items[0]?.itemName && values.items[0]?.price && values.items[0]?.img ? " bg-white" : " bg-white/50 pointer-events-none"}
                    `}
                >
                  Draft
                </button>
              ...
              <motion.div whileHover={{ scale: 0.98 }}>
                <button
                  className={` py-3 text-white rounded-md hover:bg-white hover:text-primary
                    ${values.items[0]?.itemName && values.items[0]?.price && values.items[0]?.img ? " bg-primary border border-primary" : " bg-primary/50 pointer-events-none"}
                    `}
                >
                  Publish Wishlist
                </button>
